<div id="question_textbox_{{ $question->id }}" class="d-flex classjustifstart">
    <textarea maxlength="5" rows="10" onkeyup="changeTextBox({{ $question->id }},'textbox')"
        ...
        class="form-control @if (!empty($exam_result->point)) @if (answer_result_true_or_false($question->id, $value->id) == true) true @else false @endif @endif ">{{ !empty($exam_result->point) && !empty($question->correctAnswer()) && isset($question->correctAnswer()->answer) && !empty($question->correctAnswer()->answer) ? strip_tags_with_whitespace($question->correctAnswer()->answer) : null }}</textarea>
