    ...
    <div class="row wrapper border-bottom white-bg page-heading">
        <div class="col-lg-10">
            ...
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item">
                    <a href="{{ route('blogs.index') }}">Bloqlar</a>
                </li>
            </ol>
        </div>
    </div>
                    ...
                    <div class="ibox-title">
                        ...
                        <div class="row align-items-center">
                            ...
                            <div class="col-6 text-right">
                                <a href="{{ route('blogs.index') }}" class="btn btn-w-m btn-default">Geri</a>
                            </div>
                        </div>
                    </div>
                    <div class="ibox-content">
                        <form
                            @if (isset($data) && !empty($data) && isset($data->id)) action="{{ route('blogs.update', $data->id) }}" @else action="{{ route('blogs.store') }}" @endif
                                        ...
                                        <div class="col-md-12">
                                            <div class="form-group">
                                                ...
                                                <textarea name="az_description"
                                                    class="form-control summernote {{ $errors->first('az_description') ? 'is-invalid' : '' }}" rows="4">{{ old('az_description', isset($data) && !empty($data) && isset($data->description['az_description']) && !empty($data->description['az_description']) ? $data->description['az_description'] : null) }}</textarea>
                                            </div>
                                        </div>
                                        ...
                                        @if (isset($data) && !empty($data) && !empty($data->images))
                                            <div class="row px-3">
                                                @foreach ($data->images as $image)
                                                    <div class="col-md-3 m-1 p-relative" style="border:1px solid #ccc" id="{{ $image }}">
                                                        <button type="button" class="btn btn-danger p-absolute"
                                                            style="top:0;right:0;"
                                                            onclick="deleteimage({{ $data->id }},'{{ $image }}')"><i
                                                                class="fa fa-trash"></i></button>
                                                        <img src="{{ getImageUrl($image, 'blogs') }}" alt="image"
                                                            width="150" class="img-fluid img-responsive">
                                                    </div>
                                                @endforeach
                                            </div>
                                            <br />
                                        @endif
                                        <input type="file" multiple name="images[]" class="form-conrol">
                                    </div>
                                </div>
                                <div class="col-md-2">
                                    <div class="form-group">
