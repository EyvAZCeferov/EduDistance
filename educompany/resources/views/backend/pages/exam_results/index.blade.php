                                ...
                                <form action="">
                                    <div class="row">
                                        <div class="col-3">
                                            ...
                                            <input type="text" name="date" autocomplete="off" value="{{ request('date') }}" class="date-range-picker form-control">
                                        </div>
                                        <div class="col-3">
                                            <label for="">Kateqoriya</label>
                                            <select name="category" class="form-control {{ $errors->first('category_id') ? 'is-invalid' : '' }}">
                                                ...
                                                @foreach(\App\Models\Category::whereNull('parent_id')->with('sub')->get() as $category)
                                                    <optgroup label="{{ $category->name['az_name'] }}">
                                                        @foreach($category->sub as $sub)
                                                            <option {{ request('category') == $sub->id ? 'selected' : '' }} value="{{ $sub->id }}">{{ $sub->name['az_name'] }}</option>
                                                        @endforeach
                                                    </optgroup>
                                                @endforeach
                                            </select>
                                        ...
                                        <div class="col-3">
                                            ...
                                            <select name="user" class="form-control {{ $errors->first('category_id') ? 'is-invalid' : '' }}">
                                                <option hidden disabled selected>Seçim edin</option>
                        ...
                        <table class="table table-bordered table-hover dataTables-base" data-order="2">
                            ...
                            @foreach($results as $data)
                                <tr class="gradeX">
                                    ...
                                    <td class="text-right">
                                        <a href="{{ route('exam.result.show', $data->id) }}" class="btn btn-warning btn-sm">Ətraflı</a>
                                    </td>
                                </tr>
                            @endforeach
